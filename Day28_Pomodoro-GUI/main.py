...
# ---------------------------- TIMER RESET ------------------------------- #
def reset_timer():
    window.after_cancel(timer)  # Cancel the timer that was run previously
...
timer_text = canvas.create_text(110, 132, text="00:00", fill="white", font=(FONT_NAME, 30, "bold"))
# canvas.pack()
canvas.grid(column=1, row=1)

label_timer = Label(text="Timer", font=(FONT_NAME, 35, "bold"), fg=GREEN, bg=YELLOW)
label_timer.grid(column=1, row=0)
label_check = Label(text="", font=(FONT_NAME, 35, "bold"), fg=GREEN, bg=YELLOW)
